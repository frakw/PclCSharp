#include "PclSharp_OCTDenoising.h"

//PointCloud to Mesh using greedy projection
HEAD pcl::PolygonMesh* CallingConvention greedyProjection(pcl::PointCloud<pcl::PointXYZ>* cloud,int k = 20,double searchRadius = 0.025,double mu = 2.5,
int maxNearestNeighbors = 100,double maxSurfaceAngle = 45,double minAngle = 10,double maxAngle = 120,bool normalConsistency = false)
{
	...
	*/
	...
	gp3.setMaximumSurfaceAngle(M_PI * maxSurfaceAngle); // 45 degrees
	gp3.setMinimumAngle(M_PI * minAngle); // 10 degrees
	gp3.setMaximumAngle(M_PI * maxAngle); // 120 degrees
